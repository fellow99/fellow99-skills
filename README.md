# fellow99-skills

[English](./README.md) | 中文

fellow99 的 Claude Code 技能集，用于日常工作自动化和提升效率。

## 可用技能

| 技能 | 说明 | 触发场景 |
|------|------|----------|
| **opencode-api** | 通过 HTTP REST API 与 opencode 服务器交互 | 需要操作 opencode 会话、发送消息、管理文件、查询配置等 |
| **千问 - 文生图** | 使用阿里云百炼千问 API 生成图像 | 需要生成图片、创建海报、插画、封面图等视觉内容 |

> 💡 每个技能的详细用法请参考 `skills/<skill-name>/SKILL.md` 和 `skills/<skill-name>/README.md`

## 前置要求

- 已安装 Node.js 环境
- 能够运行 `npx bun` 命令

## 安装

### 快速安装（推荐）

```bash
npx skills add fellow99/fellow99-skills
```

### 发布到 ClawHub / OpenClaw

本仓库支持将每个 `skills/*` 目录作为独立 ClawHub skill 发布。

```bash
# 预览将要发布的变更
./scripts/sync-clawhub.sh --dry-run

# 发布 ./skills 下所有已变更的 skill
./scripts/sync-clawhub.sh --all
```

ClawHub 按"单个 skill"安装，不是把整个 marketplace 一次性装进去。发布后，用户可以按需安装：

```bash
clawhub install opencode-api
clawhub install qwen-image
```

根据 ClawHub 的 registry 规则，发布到 ClawHub 的 skill 会以 `MIT-0` 许可分发。

### 注册插件市场

在 Claude Code 中运行：

```bash
/plugin marketplace add fellow99/fellow99-skills
```

### 安装技能

**方式一：通过浏览界面**

1. 选择 **Browse and install plugins**
2. 选择 **fellow99-skills**
3. 选择 **fellow99-skills** 插件
4. 选择 **Install now**

**方式二：直接安装**

```bash
# 安装 marketplace 中唯一的插件
/plugin install fellow99-skills@fellow99-skills
```

**方式三：告诉 Agent**

直接告诉 Claude Code：

> 请帮我安装 github.com/fellow99/fellow99-skills 中的 Skills

## 更新技能

更新技能到最新版本：

1. 在 Claude Code 中运行 `/plugin`
2. 切换到 **Marketplaces** 标签页（使用方向键或 Tab）
3. 选择 **fellow99-skills**
4. 选择 **Update marketplace**

也可以选择 **Enable auto-update** 启用自动更新，每次启动时自动获取最新版本。

![更新技能](./screenshots/update-plugins.png)

## 开发新技能

### 目录结构

每个技能应放在 `skills/<skill-name>/` 目录下，建议包含以下文件：

```
skills/<skill-name>/
├── SKILL.md           # 必需：技能定义文档（YAML Front Matter + 详细说明）
├── README.md          # 推荐：用户使用文档
├── *.sh               # 可选：Bash 脚本封装
├── *.js               # 可选：JavaScript 客户端
└── *.py               # 可选：Python 客户端
```

### SKILL.md 模板

```markdown
---
name: 技能名称
description: 简短描述（用于技能列表展示）
compatibility: 依赖的环境变量或工具
---

# 技能标题

## 触发时机

说明什么情况下应该使用此技能。

## 前置条件

列出使用此技能前需要准备的环境变量、API Key 等。

## 使用方法

提供详细的使用说明和示例。

## 最佳实践

分享使用该技能的建议和注意事项。
```

### 发布流程

1. 在 `skills/` 下创建新技能目录
2. 编写 `SKILL.md` 和 `README.md`
3. 运行 `./scripts/sync-clawhub.sh --dry-run` 预览
4. 运行 `./scripts/sync-clawhub.sh --all` 发布

## 许可证

本仓库中的技能以 `MIT-0` 许可分发（通过 ClawHub 发布时）。

## 贡献

欢迎提交 Issue 和 Pull Request 来改进现有技能或添加新技能！
