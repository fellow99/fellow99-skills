...
## Error Handling (Production Rules)

> 以下规则会被 [`code-review.md`](code-review.md) 中的 ArkTS 语言章节强制检查。
...
try {
  ...
  hilog.error(DOMAIN, TAG, 'File open failed: %{public}s', JSON.stringify(e));
}
```

### 异步 API with await：try-catch 覆盖 throw 和 rejection
...
try {
  ...
  hilog.error(DOMAIN, TAG, 'DB init failed: code=%{public}d', err.code);
}
```

### 异步 API with Promise：必须有 .catch() 或 onRejected

```typescript
// ❌ 禁止：Promise 链无错误处理
httpRequest.request(url, options).then((resp) => { /* ... */ });

// ✅ 正确
httpRequest.request(url, options)
  .then((resp) => { /* ... */ })
  .catch((e: BusinessError) => {
    hilog.error(DOMAIN, TAG, 'Request failed: code=%{public}d', e.code);
  });
```
...
try {
  ...
  const err = e as BusinessError;
  switch (err.code) {
    case 16000050:  // Device offline
      showToast('设备离线');
      break;
    case 16000051:  // Network error
      showToast('网络连接失败');
      break;
    default:
      hilog.error(DOMAIN, TAG, 'Unexpected: code=%{public}d, msg=%{public}s',
...
// ✅ 正确：至少记录
try { await riskyOp(); } catch (e) {
  hilog.warn(DOMAIN, TAG, 'Non-critical op failed: %{public}s', JSON.stringify(e));
}
```
